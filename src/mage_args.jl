...
function add_utcgp_env_to_args!(args)
    args["utcgp_constrained"] = get(ENV, "UTCGP_CONSTRAINED", "")
    args["utcgp_min_int"] = get(ENV, "UTCGP_MIN_INT", "")
    args["utcgp_max_int"] = get(ENV, "UTCGP_MAX_INT", "")
    args["utcgp_min_float"] = get(ENV, "UTCGP_MIN_FLOAT", "")
    args["utcgp_max_float"] = get(ENV, "UTCGP_MAX_FLOAT", "")
    args["utcgp_small_array"] = get(ENV, "UTCGP_SMALL_ARRAY", "")
    args["utcgp_big_array"] = get(ENV, "UTCGP_BIG_ARRAY", "")
    return args
end
