...
add_utcgp_env_to_args!(Parsed_args)
@show Parsed_args
...
time_add_procs = @elapsed USE_SKI ? addprocs(nt, exeflags = ["--threads=1", "--sysimage=/opt/sysimg.so"]) : nothing
println("TIME ADDING PROCS $time_add_procs")

st_time_everywhere = time()
@everywhere using UTCGP, MAGE_SKIMAGE_MEASURE, Images
end_time_everywhere = time()
println("TIME Running @everywhere $(st_time_everywhere - end_time_everywhere)")
...
trainy = Float64.(trainy)
print(trainy |> histogram)
...
if length(unique(trainy)) == 1
    @warn "Data is constant so will not fit."
    exit()
end
