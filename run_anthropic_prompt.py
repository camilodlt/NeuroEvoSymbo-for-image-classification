#!/usr/bin/env python3
"""Compose the LLM grading prompt and save Anthropic's JSON response."""
...
def strip_code_fences(text: str) -> str:
    """Remove a surrounding markdown JSON code fence if the model emits one."""
    stripped = text.strip()
...
def call_anthropic(prompt: str, model: str, api_key: str, max_tokens: int, retries: int) -> str:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            last_error = e
            print(f"[warn] Anthropic stream closed early on attempt {attempt}/{retries}: {e}")
        except httpx.ReadTimeout as e:
            last_error = e
            print(f"[warn] Anthropic read timeout on attempt {attempt}/{retries}: {e}")
        except httpx.TransportError as e:
            last_error = e
            print(f"[warn] Anthropic transport error on attempt {attempt}/{retries}: {e}")
        if attempt < retries:
...
def main() -> None:
    """Compose the prompt, call Anthropic, and save validated JSON output."""
    args = parse_args()
    ...
    if args.method == "bigram" and args.library_id is None:
        ...
        for library_id in [1, 2, 3, 4]:
            ...
            print(prompt)
            print("===== PROMPT END =====")
            raw_text = call_anthropic(prompt, args.model, api_key, args.max_tokens, args.retries)
